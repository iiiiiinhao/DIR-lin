...
def view_diff(m, r, w):
    ...
    # w = w.transpose((2, 0, 1))
    # print('m',m.shape)
    # x = np.zeros([256,256,96],dtype=float)
...
#    ad_diff1 = test_utils.DiffAdjust(diff1[index])
    ...
    print('r', r.dtype, 'm', m.dtype, 'w', w.dtype, 'ad_diff0', ad_diff0.dtype, 'y', ad_diff1.dtype)

    fig = plt.figure()
    ax1 = fig.add_subplot(2,3,1)
    ax1.imshow(r[:,145,:], plt.cm.gray)
    # ax1.imshow(r[index], 'gray')
    # ax1.set_title('固定图像,F')
    ax1.axis('off')

    ax2 = fig.add_subplot(2,3,2)
    ax2.imshow(m[:,145,:], plt.cm.gray)
    # ax2.imshow(m[index], 'gray')
    # ax2.set_title("浮动图像,M")
    ax2.axis('off')

    ax3 = fig.add_subplot(2,3,3)
    ax3.imshow(w[:,145,:], plt.cm.gray)
    # ax3.imshow(w[index], 'gray')
    # ax3.set_title("配准图像,M(φ)")
    ax3.axis('off')

    ax5 = fig.add_subplot(2,3,5)
    ax5.imshow(ad_diff0, plt.cm.gray)
    # ax5.set_title("配准前,M-F")
    ax5.axis('off')

    ax6 = fig.add_subplot(2,3,6)
    ax6.imshow(ad_diff1, plt.cm.gray)
    # ax6.set_title("配准后,M(φ)-F")
    ax6.axis('off')
