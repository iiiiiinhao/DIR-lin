...
model = model.snet(ndown=3, img_size=img_size).cuda()
...
weights_fname = 'weights-val8_plus-att-100-0.835-0.835.pth'

#root = '../dir/data3/'
for case in range(1,7):
    Transform = torchvision.transforms.Compose([transform.OneNorm(),
                                                transform.ToTensor()])
    # path = 'E:/dir_lab_pre/dirlab_Va1_plus/val/'
    ...
    #save flow
    if en:
        flow_fname = 'flow_en_val%g_plus_case%g_lamb%g_reso2_noseg.npy' % (val_index,case, lamb)
    else:
